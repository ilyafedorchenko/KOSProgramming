...
FUNCTION CIRC_MNV {
	...
	LOCK CIRCDATA TO APOBurn(). //Достаем данные по тангажу и прочее из функции ApoBurn

	UNTIL CIRCDATA[4]<0	{
		
		LOCK THROTTLE to MIN (CIRCDATA[4]/100, 1). // Управляем тягой, если осталось меньше 100dV, то снижаем пропорционально
		LOCK STEERING to PROGRADE + R(CIRCDATA[0],0,0). //Угол тагажа выставляем по данным, возвращенным функцией.
		
		// Чего возвращает ApoBurn
		//				0	1	2   3     4    5 			
		//	RETURN LIST(Fi, Vh, Vz, Vorb, dVh, DeltaA).	
		print "Fi: "		+	CIRCDATA[0] AT(0,2).	
		print "Vh: "		+	CIRCDATA[1] AT(0,3).
		print "Vz: "		+	CIRCDATA[2] AT(0,4).	
		print "Vorb: "		+	CIRCDATA[3] AT(0,5).	
		print "dVh: "		+	CIRCDATA[4] AT(0,6).		
		print "DeltaA: "	+	CIRCDATA[5] AT(0,7).	
	WAIT 0.
	}
